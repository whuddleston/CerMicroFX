...
df_particles_fine %>%
  ...
  filter(Label != "WH19") %>%
  ggplot() +
  geom_density_ridges(size = 1, aes(x = R3, y = Ni.Vol.Corrected, group = Labelfine, 
                                    #color = black,
                                    fill = as.factor(Temp))) +
  #
  geom_vline(xintercept = 10^as.numeric(c(t(as.vector(prcomp.R3.pcsx))))[1], 
  ...
  scale_x_log10(sec.axis = sec_axis(~.^(1/3)*2, name = expression(D~'['*mu*m*']')),
                ...
                breaks = scales::trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  scale_y_continuous(breaks = c(0.05,0.1,0.2,0.24,0.28,0.35,0.40,0.50), sec.axis = sec_axis(~., labels = NULL)) +
  #
  {if(cb == 1)scale_fill_scico_d(palette = "batlow", direction = 1, begin = 0, end = 0.7)} +
  #
  #geom_hline(yintercept = 0, color = "White", size = 1.5) +
...
prcomp.R3.rot %>%
  ...
  #
  ggplot() +
  geom_tile(aes(x = r3x, y = 1, height = 0.1, fill = PC1)) +
  geom_tile(aes(x = r3x, y = 2, height = 0.1, fill = PC2)) +
  geom_tile(aes(x = r3x, y = 3, height = 0.1, fill = PC3)) +
  geom_tile(aes(x = r3x, y = 4, height = 0.1, fill = PC4)) +
  geom_tile(aes(x = r3x, y = 5, height = 0.1, fill = PC5)) +
  #
...
PCANi <- r3densities %>%
  ...
  geom_ridgeline_gradient(size = 1, aes(x = r3x, y = ni2-1, 
                              group = Labelfine, 
                              height = density*5)) +
  geom_tile(aes(x = r3x, y = 9, height = 0.1)) +
  scale_fill_scico(palette = "roma", limits = c(-0.15,0.15), direction = -1) +
  #scale_fill_gradientn(colors = brewer_pal(palette = "RdBu")(5), limits = c(-0.15,0.15)) +
  scale_x_log10(sec.axis = sec_axis(~.^(1/3)*2, name = expression(D~'['*mu*m*']')),
                limits = c(0.0001, 10000),
...
PCAhrs <- r3densities %>%
  ...
  geom_ridgeline_gradient(size = 1, aes(x = r3x, y = hrs2, 
                              group = Labelfine, 
                              height = density*200)) +
  geom_tile(aes(x = r3x, y = 270, height = 10)) +
  scale_fill_scico(palette = "roma", limits = c(-0.15,0.15), direction = -1) +
  #scale_fill_gradientn(colors = brewer_pal(palette = "RdBu")(5), limits = c(-0.15,0.15)) +
  scale_x_log10(sec.axis = sec_axis(~.^(1/3)*2, name = expression(D~'['*mu*m*']')),
                limits = c(0.0001, 10000),
...
for (i in c("PC1","PC2","PC3","PC4","PC5")) {
  ...
  geom_ridgeline_gradient(size = 1, aes(x = r3x, y = Temp, 
                              group = Labelfine, 
                              height = density*500)) +
  geom_tile(aes(x = r3x, y = 1500, height = 10)) +
  scale_fill_scico(palette = "roma", limits = c(-0.15,0.15), direction = -1) +
  #scale_fill_gradientn(colors = brewer_pal(palette = "RdBu")(5), limits = c(-0.15,0.15)) +
  scale_x_log10(sec.axis = sec_axis(~.^(1/3)*2, name = expression(D~'['*mu*m*']')),
                limits = c(0.0001, 10000),
