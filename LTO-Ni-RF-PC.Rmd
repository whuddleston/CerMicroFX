...
PCRF.hyperfull <- depth.data %>%
  ...
  scale_x_continuous(labels = NULL, sec.axis = sec_axis(~., labels = NULL)) +
  scale_y_reverse(breaks = c(1:29), labels = c(pc_ranks %>% select(pcid) %>% pull(),"","nodesize","ntree","mtry","trainfrac"),
                     sec.axis = sec_axis(~., breaks = c(1:29), labels = NULL)) +
  ylab("") +
  xlab("") +
  ...
  guides(fill = "none") +
  new_scale_fill() +
  geom_tile(aes(x = modelrank, y = 26, fill = nodesize.int)) +
  scale_fill_scico(palette = "broc", direction = -1, begin = 1, end = 0.5) +
  guides(fill = "none") +
  new_scale_fill() +
  geom_tile(aes(x = modelrank, y = 27, fill = ntree.int)) +
  scale_fill_scico(palette = "cork", direction = -1, begin = 1, end = 0.5) +
  guides(fill = "none") +
  new_scale_fill() +
  geom_tile(aes(x = modelrank, y = 28, fill = mtry.int)) +
  scale_fill_scico(palette = "vik", direction = -1, begin = 0, end = 0.5) +
  guides(fill = "none") +
  new_scale_fill() +
  geom_tile(aes(x = modelrank, y = 29, fill = trainfrac)) +
  scale_fill_scico(palette = "vik", direction = 1, begin = 0.5, end = 1) +
