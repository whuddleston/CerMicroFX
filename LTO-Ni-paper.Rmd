...
voronoiTempave <- voronoidens %>%
  ...
  annotation_custom(text_grob(line1, color = "purple", rot = 90, face = "bold", size = 14), 
                    xmin = 540, xmax = 540, ymin = 1.25, ymax = 1.25) +
  annotation_custom(text_grob(line2, color = "red", rot = 90, face = "bold" , size = 14),
...
<!--
```{r}
#must run both PCA files (r3d and nnd/vrn) to run this chunk
nudge <- 0.1
...
dens.space.ni <- voronoidens %>%
  ...
  mutate(nndx = vx) %>%
  left_join(., prcomp.NND.rot, by = "nndx") %>%
  ...
  #                    height = density*0.35, #height is either density or count
  #                    group = Labelfine)) + 
  geom_ridgeline_gradient(aes(x = x, y = Ni.Vol.Corrected2, fill = vrn_PC1,
                     #fill = as.factor(hrs),
                     #height = count/20000,
                     min_height = -1000,
                     height = -density*0.55, #height is either density or count
                     group = Labelfine)) +
  geom_tile(aes(x = x, y = 0, width = 0.025, height = 0.25, fill = vrn_PC1)) +
  scale_fill_scico(palette = "roma", limits = c(-0.2,0.2), 
                   direction = -1) +
  guides(color = "none", fill = guide_colourbar(reverse = TRUE, title = "vrn_PC1", 
                               #override.aes = list(size=4), 
                               title.theme = element_text(colour = "black",
                                                          #face = "bold",
                                                          size = 12))) +
  ...
  geom_boxplot(color = "black", #fill = "purple", 
               width = 0.1, aes(group = Ni.Vol.Corrected, y = Ni.Vol.Corrected2-nudge, x = avevoronoix)) +
  geom_point(size = 1, shape = 21, #fill = "purple", 
             aes(x = avevoronoix, y = Ni.Vol.Corrected2-nudge, 
                 #size = as.factor(hrs),
                 shape = as.factor(Temp),
                 #color = as.factor(Ni.Vol.Corrected)
                 )) +
  ...
  #geom_point(size = 1, shape = 22, fill = "red", aes(y = Ni.Vol.Corrected2+nudge, x = fineavennd)) +
  #
  scale_x_log10(limits = c(0.2,3), sec.axis = sec_axis(~., labels = NULL)) +
  scale_y_continuous(breaks = c(0:8),
                     labels = c("PC1","0.05", "0.10", "0.20", "0.24", "0.28", "0.35", "0.40", "0.50"),
                     sec.axis = sec_axis(~., breaks = c(1:8), labels = NULL)) +
  #scale_fill_manual(values=c("#00BF7D", "#BF7D00", "#7C00BF", "#BF0041")) +
  #
  ylab("Nickel Volume Fraction") +
...
print(dens.space.ni)
{if(save == 1)ggsave(filename = "./paperexports/dens_space-cts-ni-grad.png", plot = dens.space.ni, 
    device = "png", width = 6,height = 4, units = "in")}
rm(dens.space.ni)
```
...
PAL2 <- scico(PALn, begin = PALs, end = PALe, direction = PALDir, palette = PAL)[2]
PAL3 <- scico(PALn, begin = PALs, end = PALe, direction = PALDir, palette = PAL)[3]
PAL4 <- scico(PALn, begin = PALs, end = PALe, direction = PALDir, palette = PAL)[4]
```


```{r}
CNN_preds <- read.csv("./data-files/preds0_class_correct.csv") %>%
  mutate(correct_fine = paste(correct,obs_class,sep = " ")) %>%
  ggplot() +
  geom_point(position = "jitter", size = 2, shape = 21, aes(x = Ni.Vol.Corrected, y = condAve, fill = as.factor(correct))) +
  geom_hline(yintercept = 10, color = "red",linetype = "dashed", size = 1) +
  #annotate("rect", ymin = .0001, ymax = 1, xmin = -Inf, xmax = Inf, fill="red", alpha = 0.1) +
  #annotate("rect", ymin = 1, ymax = Inf, xmin = -Inf, xmax = Inf, fill="blue", alpha = 0.1) +
  scale_y_log10(limits = c(0.005, 90000), 
                breaks = scales::trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)),
                sec.axis = sec_axis(~., labels = NULL)) +
  scale_x_continuous(sec.axis = sec_axis(~., labels = NULL)) +
  scale_fill_manual(values = c(PAL1,PAL3), labels = c("Correct","Incorrect")) +
  xlab("Nickel Volume Fraction") +
  ylab("Conductivity [S/cm]") +
...
print(CNN_preds)
{if(save == 1)ggsave(filename = "./paperexports/CNN_preds.png", plot = CNN_preds, 
    device = "png", width = 6,height = 4, units = "in")}
rm(CNN_preds)
```
