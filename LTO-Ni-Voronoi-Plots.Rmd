...
voronoidens %>%
  ...
  #
  ggplot() +
  geom_point(size = 2, aes(y = log(stresso), x = avevcoarse,
                           shape = as.factor(hrs),
                           color = as.factor(Temp))) +
  ...
  #            aes(x = avevoronoix,
  #                y = stressf)) +
  scale_y_continuous(limits = c(3, 7), sec.axis = sec_axis(~., labels = NULL)) +
  scale_x_continuous(limits = c(0.25, 2), sec.axis = sec_axis(~., labels = NULL)) +
  #guides(shape = FALSE) +
  #guides(color = FALSE) +
  theme_classic() +
  ...
  guides(shape = guide_legend(title = "Time [hrs]", reverse = TRUE,
                               ...
                               title.theme = element_text(colour="black",
                                                          #face = "bold",
                                                          size = 12))) +
  guides(fill = FALSE, colour = guide_legend(title = expression(Temp~'['*degree*C*']'), reverse = TRUE,
                               #override.aes = list(size=4), 
                               title.theme = element_text(colour="black",
                                                          #face = "bold",
                                                          size = 12))) +
