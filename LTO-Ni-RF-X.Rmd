...
XRF.hyperlegend <- model.meta.results.x %>%
  ...
  ggplot() +
  geom_tile(aes(x = modelrank, y = 2, fill = ntree.int)) +
  scale_fill_scico(palette = "cork", direction = -1, begin = 1, end = 0.5) +
  
  new_scale_fill() +
  geom_tile(aes(x = modelrank, y = 1, fill = mtry.int)) +
  scale_fill_scico(palette = "vik", direction = -1, begin = 0, end = 0.5) +
  
  new_scale_fill() +
  geom_tile(aes(x = modelrank, y = 3, fill = nodesize.int)) +
  scale_fill_scico(palette = "broc", direction = 1, begin = 0.5, end = 1) +
  
  new_scale_fill() +
  geom_tile(aes(x = modelrank, y = 0, fill = trainfrac)) +
  scale_fill_scico(palette = "vik", direction = 1, begin = 0.5, end = 1) +
  
  
  scale_x_continuous(labels = NULL, sec.axis = sec_axis(~., labels = NULL)) +
  scale_y_continuous(breaks = c(0,3,1,2), labels = c("trainfrac","nodesize","mtry","ntree"), sec.axis = sec_axis(~., labels = NULL)) +
  ylab("") +
  xlab("") +
  theme_classic() +
...
x_ranks <- x_ranks %>%
  mutate(xclean = NA) %>%
  mutate(xclean = if_else(xid == "ave.ave.branches", "Average Branch Length", xclean)) %>%
  mutate(xclean = if_else(xid == "ave.max.branches", "Maximum Branch Length", xclean)) %>%
  mutate(xclean = if_else(xid == "ave.branches", "Average Number of Branches", xclean)) %>%
  mutate(xclean = if_else(xid == "aveAreafine", "Average Particle Area", xclean)) %>%
  mutate(xclean = if_else(xid == "avevoronoix", "Average Edge-to-Edge Spacing", xclean)) %>%
  mutate(xclean = if_else(xid == "fineavennd", "Average Center-to-Center Spacing", xclean))
```
...
XRF.hyperfull <- depth.data.x %>%
  ...
  scale_x_continuous(labels = NULL, sec.axis = sec_axis(~., labels = NULL)) +
  scale_y_reverse(breaks = c(1:6,7,7.5,8,8.5), labels = c(x_ranks %>% select(xclean) %>% pull(),"nodesize","ntree","mtry","trainfrac"),
                     sec.axis = sec_axis(~., breaks = c(1:6,7,7.5,8,8.5), labels = NULL)) +
  ylab("") +
  xlab("") +
  ...
  guides(fill = "none") +
  new_scale_fill() +
  geom_tile(height = 0.5, aes(x = modelrank, y = 7, fill = nodesize.int)) +
  scale_fill_scico(palette = "broc", direction = -1, begin = 1, end = 0.5) +
  guides(fill = "none") +
  new_scale_fill() +
  geom_tile(height = 0.5, aes(x = modelrank, y = 7.5, fill = ntree.int)) +
  scale_fill_scico(palette = "cork", direction = -1, begin = 1, end = 0.5) +
  guides(fill = "none") +
  new_scale_fill() +
  geom_tile(height = 0.5, aes(x = modelrank, y = 8, fill = mtry.int)) +
  scale_fill_scico(palette = "vik", direction = -1, begin = 0, end = 0.5) +
  guides(fill = "none") +
  new_scale_fill() +
  geom_tile(height = 0.5, aes(x = modelrank, y = 8.5, fill = trainfrac)) +
  scale_fill_scico(palette = "vik", direction = 1, begin = 0.5, end = 1) +
...
XRF.hyperdepth <- depth.data.x %>%
  ...
  scale_fill_scico(palette = "glasgow", direction = -1, begin = 0, end = 1) +
  scale_x_continuous(labels = NULL, sec.axis = sec_axis(~., labels = NULL)) +
  scale_y_reverse(breaks = c(1:10), labels = c(x_ranks %>% select(xclean) %>% pull(),"","ntree","mtry","trainfrac"),
...
RF.default.comparison <- model.meta.results.x %>%
  ...
  ylab(expression(Testing ~ ~ Adj. ~ R^2)) +
  xlab("Model Rank") +
